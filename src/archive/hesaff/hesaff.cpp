   ...
   void exportKeypoints(ostream &out, InputOutputArray &outImage)
      {
         ...
         for (size_t i=0; i<keys.size(); i++)
         {
            ...
            {
               out << " " << int(k.desc[i]);
               circle(outImage, Point(k.x, k.y), 3, Scalar(0, 0, 255), 1);
            }
            out << endl;
         }
      }
...
{
   // Author source code
   string name = "kittens2.jpg";  // Edit this to get output of a different image
   Mat tmp = imread(name);
   ...
   double t1 = 0;
   {
      ...
      cout << "Detected " << g_numberOfPoints << " keypoints and "  // Comment this line out for non-UNIX sys
         << g_numberOfAffinePoints << " affine shapes in " << getTime()-t1 << " sec." << endl;
